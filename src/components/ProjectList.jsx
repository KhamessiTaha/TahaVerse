...
const ProjectList = ({ projects, onProjectSelect, selectedProject }) => {
  ...
  const filteredProjects = useCallback(() => {
    ...
    return projects.filter(project => {
      ...
      const statusMatch = (Array.isArray(project.status) ? project.status : [project.status])
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col items-start">
            <span className="text-sm font-bold text-cyan-300">Mission Control</span>
                    ...
                    {(Array.isArray(project.status) ? project.status : [project.status]).map((status, i) => (
                      <span
                        ...
                        className={`px-2 py-1 rounded-md text-xs font-medium border transition-colors ${
                          status === 'Live' ? 'bg-green-500/20 text-green-400 border-green-500/50' :
                          status === 'Development' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/50' :
                          status === 'Complete' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/50' :
                          status === 'Beta' || status === 'Beta Testing' ? 'bg-blue-500/20 text-blue-400 border-blue-500/50' :
                          status === 'Alpha' ? 'bg-orange-500/20 text-orange-400 border-orange-500/50' :
                          status === 'Featured' || status === 'Award Winner' ? 'bg-purple-500/20 text-purple-400 border-purple-500/50' :
